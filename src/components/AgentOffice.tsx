  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      statesRef.current.forEach((s) => {
        ...
        // 交通灯
        const lights = ['#f85149', '#e3b341', '#3fb950'];
        lights.forEach((c, i) => {
          ctx.fillStyle = c;
          ctx.beginPath();
          ctx.arc(bx + 9 + i * 12, by + 10, 3, 0, Math.PI * 2);
          ctx.fill();
        });
        ...
        ctx.fillText(`${s.symbol} ${s.shortName}`, bx + 42, by + 14);
        ...
        ctx.fillText(txt, bx + 8, by + TH + 14);
        ...
        ctx.beginPath();
        ctx.arc(bx + 8, by + TH + 30, 3.5, 0, Math.PI * 2);
        ctx.fill();
        ctx.globalAlpha = 1;

        ctx.fillStyle = st.color;
        ctx.font      = '9px "JetBrains Mono",monospace';
        ctx.fillText(`● ${st.label}`, bx + 16, by + TH + 33);
      });
